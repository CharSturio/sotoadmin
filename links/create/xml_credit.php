...
require 'call.php';
  ...
  $resultNow = mysqli_query($link,$queryNow) or die ('Consulta fallida: ' . mysqli_error($link));
  //$rowNow = mysqli_fetch_assoc($resultNow);
  $queryDandC = "SELECT D.last_date, D.invoice, D.id, D.id_invoice, D.last_digits, D.total, D.payment_method, C.rfc, C.name, C.address, C.noExt, C.noint, C.colony, C.city, C.state, C.pc, C.phone, C.email, D.usocfdi FROM documents AS D INNER JOIN clients AS C ON D.id_client = C.id WHERE D.id=" . $_REQUEST['id'] ." LIMIT 1;";

  $resultDandC = mysqli_query($link,$queryDandC) or die ('Consulta fallida: ' . mysqli_error($link));
  $rowDandC = mysqli_fetch_assoc($resultDandC);
  if (isset($rowDandC['name']) && isset($rowDandC['address']) && isset($rowDandC['colony']) && isset($rowDandC['state']) && isset($rowDandC['rfc']) && isset($rowDandC['pc']) && isset($rowDandC['city']) && isset($rowDandC['phone']) && isset($rowDandC['noExt'])) {

    //isset($fecha_ = $rowDandC['last_date'];
    $fecha_ = date("Y-m-d H:i:s", (strtotime ("-6 Hours")));
    //$fecha_ = date("Y-m-d H:i:s");
    $fecha_O = str_replace(" ", "T", $fecha_);
    $idInvoice = $rowDandC['id_invoice'];
  ...
  echo $IVAtot;
    ...
    $noCertificado = '00001000000407881077';
